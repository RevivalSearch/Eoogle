...
const EMOJIS = {
    ...
    known: '<:known:1481843025679224946>'
};
// Constants
...
client.on('messageCreate', async message => {
    ...
    if (message.author.id === ADMIN_ID) {
        ...
        if (command === 'unlinkuser') {
            ...
            let accountType = args[2] ? args[2].toLowerCase() : null;
            
            // If no type specified, try to determine from the ID format
            if (!accountType) {
                accountType = accountId.length <= 5 ? 'ecsr' : 'korone';
                message.channel.send(`⚠️ No account type specified, assuming ${accountType.toUpperCase()} ID`);
            }
            
            // Validate account type
            if (accountType !== 'ecsr' && accountType !== 'korone') {
                return message.reply('❌ Invalid account type. Must be `ecsr` or `korone`.');
            }
            
            try {
                // First, find the Discord user ID associated with this account
                const db = readDB();
                const discordUser = Object.entries(db.users || {}).find(([_, accounts]) => 
                    accounts[accountType] === accountId || 
                    accounts[accountType] === `k_${accountId}` // Handle k_ prefix for Korone
                );
                
                if (!discordUser) {
                    return message.reply(`❌ No linked Discord account found for ${accountType.toUpperCase()} ID: ${accountId}`);
                }
                
                const [discordId, userData] = discordUser;
                const result = unlinkAccount(discordId, accountType);
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`Successfully unlinked ${accountType.toUpperCase()} account ${accountId} from <@${discordId}>`)
                    .addFields({
                        name: 'Details',
